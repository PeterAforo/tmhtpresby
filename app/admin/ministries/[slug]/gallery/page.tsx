...
export default function MinistryGalleryPage({ params }: Props) {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/ministries/${slug}/gallery`);
      if (res.ok) {
  ...
  const handleBulkUpload = async () => {
    ...
    // Add images to ministry gallery
    if (uploadedUrls.length > 0) {
      try {
        await fetch(`/api/admin/ministries/${slug}/gallery`, {
          method: "POST",
  ...
  const handleDeleteImage = async (imageId: string) => {
    ...
    try {
      await fetch(`/api/admin/ministries/${slug}/gallery?imageId=${imageId}`, {
        method: "DELETE",
      });
      fetchData();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-[var(--text)]">{ministryName} Gallery</h1>
          ...
          {selectedFiles.length > 0 && (
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-sm text-[var(--text-muted)]">Selected files:</p>
                <button
      ...
      {/* Gallery Images Grid */}
      <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border)] p-6">
        <h2 className="text-lg font-semibold text-[var(--text)] mb-4">Gallery Images</h2>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            <ImageIcon size={48} className="mx-auto text-[var(--text-muted)] mb-4" />
            <p className="text-[var(--text-muted)]">No images in gallery yet</p>
